...
Hierarchical exception classes for clean error handling across all packages.
"""
...
class ValidationError(OmniSLMError):
    ...
    def __init__(
        self, message: str = "Validation error", errors: list[dict] | None = None
    ):
...
class RuntimeNotAvailableError(ModelError):
    ...
    def __init__(self, runtime: str):
        super().__init__(
            message=f"Runtime '{runtime}' is not available or not configured"
        )
...
class PluginError(OmniSLMError):
    """Base error for plugin-related issues."""

    def __init__(self, message: str = "Plugin error"):
        super().__init__(message=message, code="PLUGIN_ERROR")
...
class PluginNotFoundError(PluginError):
    ...
    def __init__(self, plugin_name: str):
        super().__init__(message=f"Plugin '{plugin_name}' not found")
...
class PluginInitializationError(PluginError):
    ...
    def __init__(self, plugin_name: str, reason: str = ""):
        msg = f"Plugin '{plugin_name}' failed to initialize"
        if reason:
            msg += f": {reason}"
        super().__init__(message=msg)
        self.code = "PLUGIN_INIT_ERROR"


# ---- Registry Errors ----


class RegistryError(OmniSLMError):
    """Raised for registry-related issues."""

    def __init__(self, message: str = "Registry error"):
        super().__init__(message=message, code="REGISTRY_ERROR")


# ---- Workflow Errors ----


class WorkflowError(OmniSLMError):
    """Base error for workflow-related issues."""

    def __init__(self, message: str = "Workflow error"):
...
class WorkflowCycleError(WorkflowError):
    ...
    def __init__(self):
        super().__init__(message="Workflow DAG contains a cycle")
        self.code = "WORKFLOW_CYCLE"


# ---- Configuration Errors ----


class ConfigurationError(OmniSLMError):
